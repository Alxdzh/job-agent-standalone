...
一个本地招聘工作台。填写 JD 判断资料和平台条件后，在可见的 Chrome 页面中登录招聘平台并执行投递。

本版本直接使用网页工作台，不接入 Agent，也不包含聊天、QQ 或微信功能。需要通过 Agent 用自然语言控制投递，请使用配套的 [求职管家 MCP 版](https://github.com/Alxdzh/job-agent-mcp)。

## 资料和投递偏好

概览中的“JD 判断资料”只保存个人背景和岗位判断补充条件，供 JD 判断参考；“设置 → 投递偏好”才是实际投递条件的唯一来源，包括城市、关键词、薪资、启用状态和筛选词。保存资料不会修改平台配置，也不会开始投递。
...
1. 在“概览”的“JD 判断资料”框填写用于 JD 判断的个人背景和岗位判断条件。
2. 在“设置”中配置城市、关键词、薪资、投递时间和随机节奏。
3. 在登录区域逐个平台打开可见浏览器，完成登录并检查登录态。
4. 手动投递选择平台和数量后点击开始；持续投递点击右上角开始按钮。
5. 在状态卡片和“投递记录”标签中查看实际结果。
