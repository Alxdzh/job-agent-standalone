...
assert.match(platformSources.zhilian, /clickStayOnCurrentPage/)
assert.match(platformSources.zhilian, /留在此页/)
assert.match(platformSources.zhilian, /evidenceAfterDelivery/)
assert.doesNotMatch(platformSources.zhilian, /findVisibleTextHandle\(page,\s*['"]继续沟通['"]|findVisibleTextHandle\([^\n]+,\s*['"]继续沟通['"]/, 'Zhilian flow must never locate a continue-chat button for clicking')
...
assert.match(platformSources.liepin, /waitForLiepinDetailPage/)
assert.match(platformSources.liepin, /beforeUrls/)
assert.match(platformSources.liepin, /new-or-changed-detail-url/)
assert.match(platformSources.liepin, /LIEPIN_DIRECT_APPLY_TEXTS/)
assert.match(platformSources.liepin, /投递简历/)
