...
function initAppShell() {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/sw.js?v=20260828-llm-model-select1').catch(() => {})
  }
}
...
async function setPlatformEnabled(platform, enabled) {
  ...
  const r = await api('POST', '/api/config', { platform, enabled })
  if (r.data?.ok) {
...
async function checkAllPlatformLogins() {
  ...
  const status = $('platform-login-status')
  if (btn) { btn.disabled = true; btn.textContent = '正在检测四个平台…' }
  status.textContent = '正在依次检测四个平台，请在可见浏览器完成扫码。'
  ...
  status.textContent = missing.length
    ? `仍有 ${missing.length} 个平台未登录，请点击对应的“打开登录”完成扫码。`
    : '四个平台均已登录，状态已保存 ✅'
  if (btn) { btn.disabled = false; btn.textContent = '重新检测四个平台登录状态' }
}
